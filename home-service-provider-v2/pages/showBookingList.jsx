...
const ShowBookingList = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://localhost:3000/booking/get', {
    ...
    const handlePayment = async (id) => {
        try {
            ...
            const response = await axios.post("http://localhost:3000/payment/insert", { id: getRandomInt(1, 1000), paymentType: "cash", paymentInfo: "pending" }, {
                withCredentials: true,
            });
            route.push("/showPaymentList");
            console.log(response);
            // setAdminList(adminList.filter((admin) => admin.id !== id));
        } catch (error) {
            console.error(error);
            ...
            <div className="flex flex-wrap justify-around">
                {adminList.map((admin) => (
                    <div key={admin.username} className="card bg-white shadow-md m-4 p-4 rounded-lg w-64">
                        ...
                        <p className="text-black">Time: {admin.time}</p>
                        <p className="text-black">Date: {admin.date}</p>
