...
const ShowServiceList = () => {
    ...
    const handleBooking = async (id) => {
        ...
        try {
            const now = new Date();
            const response = await axios.post("http://localhost:3000/booking/insert", { id: getRandomInt(1, 10000), time: now.getHours() + ":" + now.getMinutes() + ":" + now.getSeconds(), date: new Date(), ServiceId: id, UserListId: 2 }, {
                withCredentials: true,
            });
            console.log(response);
            // setAdminList(adminList.filter((admin) => admin.id !== id));
                ...
                {adminList.map((admin) => (
                    <div key={admin.username} className="card bg-white shadow-md m-4 p-4 rounded-lg w-64">
                        ...
                        <div className="flex justify-between mt-4">
                            ...
                            <button className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" onClick={() => handleBooking(admin.id)}>Book</button>
