...
const ShowManagerList = () => {
    ...
    const handleDelete = async (id) => {
        try {
            const response = await axios.delete(`http://localhost:3000/manager/delete/${id}`, {
                withCredentials: true,
            });
            console.log(response);
    ...
    return (
        <Layout status={route.query.status}>
            <div className="flex flex-wrap justify-around">
                {adminList.map((admin) => (
                    <div key={admin.username} className="card bg-white shadow-md m-4 p-4 rounded-lg w-64">
                        ...
                        <p className="text-black">Email: {admin.email}</p>
                        <p className="text-black">Date of Birth: {admin.DOB}</p>
                        <p className="text-black">Username: {admin.username}</p>
...
export default ShowManagerList;
